...
console.log("image uploaded(url): ", image? URL.createObjectURL(image):'')
  return (
<div className='h-screen overflow-scroll flex flex-col items-start
  ...
  <form  onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
    ...
    <div className='flex md:flex-row flex-col items-center gap-3'>
        ...
        <img onClick={()=>setImage(null)} className='max-h-5' src={image? assets.cross_icon: ''}
          alt="" />
        </div>
    </div>
    </div>
    ...
    <div className='flex flex-col gap-1'>
      ...
      value={discount} placeholder='0' min={0} max={100} className='outline-none
      md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500 ' required
      />
    </div>
      {/* Adding chapters and lectures */}
      <div>
        {/* will be displayed if there is atleast one chapter */}
        {chapters.map((chapter, chapterIndex)=>(
          <div key={chapterIndex} className='bg-white border rounded-lg mb-4'> 
              ...
              {!chapter.collapsed && (
                <div className='p-4'>
                  {/* will be displayed if there is atleast one lecture */}
        ...
        <div onClick={()=> handleChapter('add')} className='flex justify-center items-center bg-blue-100 p-2
                      rounded-lg cursor-pointer'>+ Add Chapter</div>
      {
       ...
       <div className='fixed inset-0 flex items-center justify-center
            ...
            <div className='mb-2'>
             ...
             className='w-full bg-blue-400 text-white py-2 px-4 rounded'> Add</button>

                 <img src={assets.cross_icon} alt="cross icon"  onClick={()=>setShowPopup(false)}
